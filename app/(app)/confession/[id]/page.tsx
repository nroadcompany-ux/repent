...
import { CONFESSION_COMMENTS_ENABLED, CONFESSION_TYPE_LABELS } from '@/domain/product-lock'
...
export default async function ConfessionDetailPage({
  ...
  searchParams: Promise<{ reported?: string; error?: string; edit?: string }>
}) {
  const { supabase, userId } = await requireUser()
  const { id } = await params
  const { reported, error, edit: editingCommentId } = await searchParams
  ...
  const [{ data: author }, { data: reactions }, { data: tags }, { data: comments }, photoUrl] = await Promise.all([
    ...
    <main>
      ...
      <article className="mx-gutter mt-3 rounded-card bg-surface px-4 py-4">
        <div className="flex items-center justify-between">
          <p className="text-caption font-medium text-ink-muted">{author?.display_name || '이름 없음'}</p>
          <p className="text-caption text-ink-faint">{formatFullDate(post.created_at.slice(0, 10))}</p>
        </div>
        ...
        {(tags ?? []).length > 0 ? (
          <ul className="mt-4 flex flex-wrap gap-2">
            {(tags ?? []).map((tag) => (
              <li key={tag.tag} className="text-caption rounded-chip bg-canvas px-[10px] py-[4px] font-medium text-ink-muted">
                #{tag.tag}
              </li>
            ))}
          </ul>
      ...
      {CONFESSION_COMMENTS_ENABLED ? (
        <section id="comments" className="mt-8">
          <h2 className="text-section px-title-gutter font-semibold text-ink">댓글 {(comments ?? []).length}</h2>

          <form action={createComment} className="mt-3 px-title-gutter">
            <input type="hidden" name="post_id" value={id} />
            <TextArea name="body" rows={3} maxLength={1000} placeholder="댓글을 남겨보세요." required />
            <div className="mt-2">
              <Button type="submit" variant="secondary">댓글 남기기</Button>
            </div>
          </form>

          {(comments ?? []).length === 0 ? (
            <p className="text-body-sm mt-4 px-title-gutter leading-[21px] text-ink-muted">아직 댓글이 없어요.</p>
          ) : (
              ...
              {(comments ?? []).map((comment) => {
                ...
                return (
                  <li key={comment.id} className="rounded-row bg-surface px-4 py-4">
                    <div className="flex items-center justify-between">
                      <p className="text-caption font-medium text-ink-muted">{commenterName.get(comment.user_id) || '이름 없음'}</p>
                      <p className="text-caption text-ink-faint">{formatMonthDay(comment.created_at.slice(0, 10))}</p>
                    </div>
                    ...
                    {editing && own ? (
                      <form action={updateComment} className="mt-2">
                        ...
                        <div className="mt-2 flex items-center gap-3">
                          <Button type="submit" variant="secondary" className="w-auto px-4">저장</Button>
                          <Link href={`/confession/${id}#comments`} className="text-caption font-medium text-ink-muted">취소</Link>
                        </div>
                      </form>
                    ) : (
                      <p className="text-body mt-2 whitespace-pre-wrap leading-[25px] text-ink">{comment.body}</p>
                    )}

                    {own && !editing ? (
                      <div className="mt-3 flex items-center gap-4">
                        <Link href={`/confession/${id}?edit=${comment.id}#comments`} className="text-caption font-medium text-ink-muted">수정</Link>
                        <form action={deleteComment}>
                          <input type="hidden" name="comment_id" value={comment.id} />
                          <input type="hidden" name="post_id" value={id} />
                          <button type="submit" className="text-caption font-medium text-ink-faint">삭제</button>
                        </form>
                      </div>
                    ) : null}
                  </li>
      ...
      <div className="mt-9 px-title-gutter">
        <Link
          href={`/confession/${id}/settings`}
          className="text-body-sm block rounded-control border border-line bg-surface px-4 py-4 text-center font-medium text-ink-muted"
        >
          설정
        </Link>
        <p className="text-caption mt-2 text-center text-ink-faint">신고·차단·글 관리는 설정에서 볼 수 있어요.</p>
      </div>
    </main>
  )
}
