...
- **Promise 1:N Action** (2026-09-05 확정) — 하나의 Promise에 여러
  Action이 연결될 수 있다
- **사용자 종료 표현 = "마무리됨"** (2026-09-05 확정) — 사용자가 직접
  Promise를 종료할 때 쓰는 사용자-facing 문구. "완료"·"성공"처럼
  성취/실패를 판정하는 표현을 쓰지 않는다
...
- Source Edit ≠ ShareCopy Auto Edit — Source를 수정해도 이미 만들어진
  ShareCopy는 자동으로 바뀌지 않는다
- **Source Delete ≠ ShareCopy Auto Delete** (신규 확정) — Source를
  삭제해도 이미 만들어진 ShareCopy는 자동으로 삭제되지 않는다
- ShareCopy Delete ≠ Source Delete — ShareCopy를 삭제해도 Source는
  삭제되지 않는다
- 공유는 **사용자 명시 선택 + Preview 확인 이후에만** 이뤄진다(자동
  공유·묵시적 공유 금지)

## Empty/Error Principle (2026-09-05, Owner/PM 확정)

- **기술적 Empty(빈 상태)/Error(오류)에 영적 의미를 부여하지 않는다**
- **허용 예시**:
  - "아직 기록이 없습니다."
...
- **금지 예시**:
  ...
  - "회개가 필요합니다."
  - "신앙 기록이 부족합니다."
- 기술적 Error는 기술적 Error로만 표현한다(영적 평가·질책으로 표현
  금지)

## Archive (2026-09-05, Owner/PM 확정)

- Archive는 **영적 완료를 의미하지 않는다** — 시스템의 보관(저장 위치
  이동) 행위일 뿐이며, "다 이루었다"·"완결됐다" 같은 영적 판정으로
  읽히는 표현과 연결하지 않는다
